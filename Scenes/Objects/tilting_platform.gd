...
func rotate_platform(direction: int, delta: float):
	if direction > 0:
		set_rotation_degrees(lerpf(rotation_degrees, max_rotation_right, 1 * delta))	
	elif direction < 0:
		set_rotation_degrees(lerpf(rotation_degrees, max_rotation_left, 1 * delta))
	else:
		set_rotation_degrees(lerpf(rotation_degrees, 0, 0.6 * delta))
